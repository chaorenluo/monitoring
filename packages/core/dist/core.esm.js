/* @monitoring/core version ' + 0.0.1 */
import { slientConsoleScope, getTimestamp, logger, validateOption, _support as _support$1, variableTypeDetection as variableTypeDetection$1, Queue, isBrowserEnv as isBrowserEnv$1, isEmpty, Severity as Severity$1, isError, extractErrorStack, unknownToString, getLocationHref, interceptStr, fromHttpStatus, SpanStatus as SpanStatus$1, toStringValidateOption, generateUUID, setSilentFlag, getFlag, setFlag, nativeTryCatch, getFunctionName } from '@monitoring/utils';
import { BREADCRUMBCATEGORYS, BREADCRUMBTYPES, ERRORTYPES, EVENTTYPES, SDK_VERSION, SDK_NAME, globalVar } from '@monitoring/shared';
...
var breadcrumb = _support$1.breadcrumb || (_support$1.breadcrumb = new Breadcrumb());
...
function objectOrder(reason) {
    var sortFn = function (obj) {
        return Object.keys(obj)
            ...
            if (variableTypeDetection$1.isObject(obj[key])) {
...
var TransportData = (function () {
    ...
    TransportData.prototype.getRecord = function () {
        var recordData = _support$1.record;
        if (recordData && variableTypeDetection$1.isArray(recordData) && recordData.length > 2) {
            return recordData;
        }
        return [];
    };
    TransportData.prototype.getDeviceInfo = function () {
        return _support$1.deviceInfo || {};
    };
    ...
    TransportData.prototype.bindOptions = function (options) {
        if (options === void 0) { options = {}; }
        var dsn = options.dsn, beforeDataReport = options.beforeDataReport, apikey = options.apikey, configReportXhr = options.configReportXhr, backTrackerId = options.backTrackerId, configReportUrl = options.configReportUrl;
...
var transportData = _support$1.transportData || (_support$1.transportData = new TransportData());

function log(_a) {
    var _b = _a.message, message = _b === void 0 ? 'emptyMsg' : _b, _c = _a.tag, tag = _c === void 0 ? '' : _c, _d = _a.level, level = _d === void 0 ? Severity$1.Critical : _d, _e = _a.ex, ex = _e === void 0 ? '' : _e;
    var errorInfo = {};
    if (isError(ex)) {
        errorInfo = extractErrorStack(ex, level);
    }
    var error = __assign({ type: ERRORTYPES.LOG_ERROR, level: level, message: unknownToString(message), name: 'monitoring.log', customTag: unknownToString(tag), time: getTimestamp(), url: getLocationHref() }, errorInfo);
    breadcrumb.push({
        type: BREADCRUMBTYPES.CUSTOMER,
        category: breadcrumb.getCategory(BREADCRUMBTYPES.CUSTOMER),
        data: message,
        level: Severity$1.fromString(level.toString())
    });
...
function handleConsole(data) {
    if (globalVar.isLogAddBreadcrumb) {
        breadcrumb.push({
            ...
            data: data,
            level: Severity$1.fromString(data.level)
        });
    }
}
var resourceMap = {
...
function httpTransform(data) {
    ...
    message = message === SpanStatus$1.Ok ? message : message + " " + getRealPath(data.url);
    return {
...
var options = _support$1.options || (_support$1.options = new Options());
...
var rrweb = createCommonjsModule(function (module, exports) {
...
var RecordScreen = (function () {
    ...
    RecordScreen.prototype.recordStart = function () {
        var newThis = this;
    ...
    RecordScreen.prototype.recordPlay = function () {
        var rePlayer = new this.record.Replayer(this.frame);
        rePlayer.play();
    };
    return RecordScreen;
}());
var recordScreen = _support.recordScreen || (_support.recordScreen = new RecordScreen());

export { Breadcrumb, Options, RecordScreen, TransportData, breadcrumb, createErrorId, getRealPath, handleConsole, hashCode, httpTransform, initOptions, log, options, recordScreen, resourceTransform, setTraceId, subscribeEvent, transportData, triggerHandlers };
