import { BREADCRUMBTYPES, ERRORTYPES, ERROR_TYPE_RE, HTTP_CODE } from '@monitoring/shared'
import { transportData, breadcrumb, resourceTransform, httpTransform } from '@monitoring/core'
import { getLocationHref, getTimestamp, isError, parseUrlToObj, extractErrorStack, unknownToString, Severity } from '@monitoring/utils'
import { ReportDataType, Replace, MITOHttp, ResourceErrorTarget } from '@monitoring/types'
const HandleEvents = {
    /**
     * 处理xhr、fetch回调
     */
    ...
    handleError:(errorEvent: ErrorEvent)=>{
        const target = errorEvent.target as unknown as ResourceErrorTarget;
        if (target.localName) {
            // 资源加载错误 提取有用数据
            const data = resourceTransform(errorEvent.target as unknown as ResourceErrorTarget)
            breadcrumb.push({
